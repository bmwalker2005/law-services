...
public class CLI extends UserInterface {
	...
	private void storeFile() {
		...
		try {
			...
			super.getDB().insertFile(f.getName(), trueData, Constants.NULL_ID, Constants.NULL_ID);
	...
	private void getLegalFile() {
		...
		try {
			LegalFile lF = super.getDB().getFile(fileID);
	...
	private void displayFiles() {
		...
		try {
			for (LegalFile file : super.getDB().getAllFiles()) {
				System.out.println("\n" + file);
				in.nextLine();
			}
		} catch (SQLException e) {
	...
	private void storeLegalCase() {
		...
		try {
			super.getDB().insertLegalCase(caseName, clientID, oppID, start, end, description, note);
	...
	private void updateLegalCase() {
		...
		try {
			...
			try {
				super.getDB().updateLegalCase(legalCaseID, caseName, clientID, oppID, start, end, description, note);
	...
	private void displayLegalCases() {
		...
		try {
			for (LegalCase legalCase : super.getDB().getAllLegalCases()) {
				System.out.println("\n" + legalCase);
				in.nextLine();
			}
		} catch (SQLException e) {
	...
	private void deleteLegalCase() {
		...
		try {
			...
			if ("y".equalsIgnoreCase(cont)) {
				try {
					super.getDB().deleteLegalCase(legalCaseID);
	...
	private void displayParties() {
		...
		try {
			for (Party party : super.getDB().getAllParties()) {
				System.out.println("\n" + party);
				in.nextLine();
			}
		} catch (SQLException e) {
	...
	private void deleteParty() {
		...
		try {
			System.out.println("Delete Party? (y to continue) \n" + super.getDB().getParty(partyID));
			String cont = in.nextLine();
			
			if ("y".equalsIgnoreCase(cont)) {
				try {
					super.getDB().deleteParty(partyID);
	...
	private void updateParty() {
		...
		try {
			...
			try {
				super.getDB().updateParty(partyID, fName, mName, lName, phone, birthDate, address, email, note);
	...
	private void storeParty() {
		...
		try {
			super.getDB().insertParty(fName, mName, lName, phone, birthDate, address, email, note);
